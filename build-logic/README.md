...
统一编译配置、插件组合、standalone 规则、架构边界、类注释校验与项目索引生成，避免各模块复制 Gradle 配置。
...
`toolkit.architecture-guard` 校验模块依赖、README、资源前缀、模板测试、项目索引和新建顶层类型的类级注释。`config/architecture/class-header-baseline.txt` 仅冻结规则启用前的历史欠账，不接受新类型例外。
