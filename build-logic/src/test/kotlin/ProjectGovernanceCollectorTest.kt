...
class ProjectGovernanceCollectorTest {
    ...
    fun `standalone assembly edge is normalized out of index but retained for architecture`() {
        val root = ProjectBuilder.builder()
            ...
            .withProjectDir(temporaryFolder.root)
            .build()
        val mineParent = ProjectBuilder.builder()
            .withName("module_mine")
            .withParent(root)
            .withProjectDir(temporaryFolder.newFolder("module_mine"))
            .build()
        val otaParent = ProjectBuilder.builder()
            .withName("module_ota")
            .withParent(root)
            .withProjectDir(temporaryFolder.newFolder("module_ota"))
            .build()
        val mineDirectory = temporaryFolder.newFolder("module_mine", "impl")
        mineDirectory.resolve("build.gradle").writeText("// feature")
        val mine = ProjectBuilder.builder()
            .withName("impl")
            .withParent(mineParent)
            .withProjectDir(mineDirectory)
            .build()
        val otaDirectory = temporaryFolder.newFolder("module_ota", "impl")
        otaDirectory.resolve("build.gradle").writeText("// feature")
        val ota = ProjectBuilder.builder()
            .withName("impl")
            .withParent(otaParent)
