...
public class ExcelUtils {
    ...
    public static <T> void writeObjListToExcel(List<T> objList, String fileName, Context c) {
        if (objList != null && objList.size() > 0 && !TextUtils.isEmpty(fileName)) {
...
//              sheet.mergeCells()
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        if (cont.length() <= 5) {
                            sheet.setColumnView(i, cont.length() + 8); //设置列宽
                        } else {
                            sheet.setColumnView(i, cont.length() + 5); //设置列宽
                        }
                    }
                ...
                writebook.write();
                LogManager.e("ExcelUtils", "导出完成");
//                Toast.makeText(c, "导出完成", Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                if (writebook != null) {
